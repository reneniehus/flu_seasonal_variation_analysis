...
.cm_long = function(fit){
  ...
  do.call(rbind, lapply(seq_len(K), function(s){
    ...
    rbind(
      data.frame(season = fit$seasons[s], week = seq_len(nrow(fit$y[[s]])), group = rep(fit$groups, each = nrow(fit$y[[s]])),
                 what = "observed", value = as.numeric(sweep(fit$y[[s]], 2, per100k, "*"))),
      data.frame(season = fit$seasons[s], week = seq_len(nrow(fit$y[[s]])), group = rep(fit$groups, each = nrow(fit$y[[s]])),
                 what = "EKF filtered (one-step-ahead)", value = as.numeric(sweep(fit$mu_filt[[s]], 2, per100k, "*"))),
      data.frame(season = fit$seasons[s], week = seq_len(nrow(fit$y[[s]])), group = rep(fit$groups, each = nrow(fit$y[[s]])),
                 what = "deterministic SIR", value = as.numeric(sweep(fit$mu_det[[s]], 2, per100k, "*"))))
...
plot_cm_fit = function(fit){
  ...
  ggplot() +
    ...
    scale_colour_manual(values = c("EKF filtered (one-step-ahead)" = "#d95f02", "deterministic SIR" = "#1b9e77"), name = NULL) +
    scale_linetype_manual(values = c("EKF filtered (one-step-ahead)" = "solid", "deterministic SIR" = "dashed"), name = NULL) +
    labs(title = sprintf("%s -- compartmental model fit: S0 = %.2f (shared across seasons), c = %.3g, b = %.1f, phi = %.1f, q = %.3f",
                         fit$country, fit$params$S0, fit$params$c, fit$params$b, fit$params$phi, fit$params$q),
...
plot_cm_wiggle = function(fit){
  ...
  ggplot(d, aes(week, group, fill = pmin(pmax(ratio, -2), 2))) + geom_tile() + facet_wrap(~ season, ncol = 4) +
    ...
    labs(title = sprintf("%s -- how much process noise each week needed (q = %.3f)", fit$country, fit$params$q),
...
plot_cm_params = function(fit){
  ...
  d = data.frame(param = nm, est = th, lo = th - 1.96 * se, hi = th + 1.96 * se, prior_mean, prior_sd, stringsAsFactors = FALSE) %>%
    mutate(family = case_when(grepl("^log_R0_", param) ~ "season R0 (log)", param == "logit_S0" ~ "S0 (logit)",
                              param %in% c("log_c", "log_b") ~ "reporting (log)", TRUE ~ "noise / dispersion (log)"),
           label = sub("^log_R0_", "", param), label = sub("^logit_", "", label), label = sub("^log_", "", label),
           label = factor(label, levels = rev(unique(label))))
  ggplot(d, aes(y = label)) +
    geom_rect(data = d %>% filter(is.finite(prior_sd)), aes(xmin = prior_mean - 1.96 * prior_sd, xmax = prior_mean + 1.96 * prior_sd,
                                                          ymin = as.numeric(label) - 0.45, ymax = as.numeric(label) + 0.45),
              fill = "#8da0cb", alpha = 0.3, inherit.aes = FALSE) +
...
save_cm_report = function(fits, dir = "output/comp_model"){
  ...
  for (fit in fits){
    ...
    ggsave(file.path(dir, sprintf("params_%s.png", fit$country)), plot_cm_params(fit), width = 10, height = 5.5, dpi = 110)
