...
cm_ekf_season_engine = function(y, f, S0, R0, c, b, phi, q, vax_day = NA, vax_frac = NULL, engine = "R"){
  if (engine == "R") return(cm_ekf_season(y, f, S0, R0, c, b, phi, q, vax_day, vax_frac))
  if (!exists("cm_ekf_season_cpp", mode = "function")) cm_load_cpp()
  cm_ekf_season_cpp(y, f$Cn, f$N, f$gamma, f$ve_inf, f$ve_spread, f$ve_ili, f$I0, f$p0, f$rate_per,
                    S0, R0, c, b, phi, q, if (is.na(vax_day)) -1L else as.integer(vax_day),
